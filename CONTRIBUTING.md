...
> [!IMPORTANT]
> This fork **accepts AI-assisted and fully AI-generated pull requests**. What we do not accept is *unverified* code, regardless of who or what wrote it. Authorship is disclosed; correctness is proven.
>
> Note: the upstream project ([ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp)) does **not** accept AI-generated contributions. Never submit work from this fork to upstream unless it independently satisfies upstream's policy - doing so can get you permanently banned there.

Requirements for every PR, human- or AI-authored:

1. **Disclose authorship.** State which AI tools/models were used and to what extent (e.g. `Assisted-by:` or `Generated-by:` trailer in commits). No disclosure needed for trivial autocomplete.
2. **Prove correctness.** Run the relevant checks before submitting and include the results in the PR description:
   - the project builds on at least one backend you tested
   - `test-backend-ops` passes if you touched `ggml` operators (add test cases for new/modified ops)
   - `llama-perplexity` shows no regression if your change can affect model output
   - `llama-bench` numbers (before/after) if the PR claims a performance improvement
3. **A human sponsor owns the PR.** Every PR must have a human account holder who submitted it, responds to review, and commits to follow-up fixes. Fully autonomous agents may author code, but not own PRs.
4. **Scope discipline.** One feature or fix per PR. Large AI-generated rewrites without prior discussion in an issue will be closed - not because AI wrote them, but because nobody can review them.

Maintainers may still close PRs that are unverified, unreviewable, or duplicative. "AI-generated" is not a defect here; "unverified" is.
